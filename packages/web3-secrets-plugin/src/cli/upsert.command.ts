...
import { encryptData } from '../lib/pkp';
...
const upsertHandler: Parameters<typeof createDmnoPluginCliCommand>[0]['handler'] = async function (
  ...
  let itemsFromConfig = _.map(ctx.plugin.usedByConfigItemResolverPaths, (fullResolverPath: string) => {
    return {
  ...
  const encryptedValue = await encryptData(vaultObj.pkpInfo, newRawValue);
